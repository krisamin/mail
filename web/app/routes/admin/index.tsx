...
export default function AdminIndex({ loaderData }: Route.ComponentProps) {
  const { domains, queueStats } = loaderData;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatCard label="도메인" value={domains.length} />
      ...
      <div className="rounded-md border border-line bg-bg-1">
        <div className="flex items-center justify-between border-b border-line px-4 py-3">
          ...
          <Link to="/admin/domains" className="text-xs text-accent hover:text-accent-hover">
            관리 →
          </Link>
        </div>
        {domains.length === 0 ? (
