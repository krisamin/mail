...
## DD-12. 권한은 자물쇠 두 개, 안쪽 문은 잠그지 않는다 (2026-09)

계정과 도메인이 각각 스위치를 갖고, **바깥으로 나가거나 바깥에서 들어오는**
경우에만 둘 다 열려 있어야 통과한다(`internal/policy`).

| 스위치 | 자리 | 뜻 |
|---|---|---|
| `can_send` | 계정 | 아예 못 보냄 (내부 포함) |
| `can_send_external` | 계정 | 서버 밖으로 못 보냄 |
| `can_receive_external` | 계정 | 서버 밖에서 오는 걸 안 받음 |
| `allow_send_external` | 도메인 | 그 도메인 전체의 발송 차단기 |
| `allow_receive_external` | 도메인 | 그 도메인 전체의 수신 차단기 |
| `daily_send_limit` | 계정 | 하루 수신자 수 상한 (UTC, NULL=무제한) |

**서버 안에서 오가는 메일은 이 스위치들이 건드리지 않는다.** "외부 차단"을
건 계정이 같은 서버 동료와도 말을 못 하게 되면 그건 권한이 아니라 격리다.
구조적으로도 내부 메일은 MX 포트를 통과하지 않고(제출·웹메일이 직배달)
바깥 판정을 받을 일이 없다.

계정 스위치를 3상태(상속/허용/차단)로 만들지 않은 이유: 도메인 스위치는
운영자의 비상 차단기라 "예외 허용"이 있으면 차단기 구실을 못 한다. AND로
묶으면 규칙이 한 줄로 설명되고("둘 다 켜져야 나간다"), 개인 예외가 필요하면
계정을 다른 도메인으로 옮기는 편이 정직하다.

집행 지점은 넷이고 전부 같은 판정 함수를 부른다: 수신 MX의 RCPT, 제출의
RCPT, 웹메일 발송 API, 그리고 발송 직전의 일일 한도. 거부는
`mail_policy_block_total{rule,path}`로 센다.

### 앱 비밀번호 스코프

`app_password.scope_list`는 0001부터 컬럼만 있고 아무도 읽지 않았다. 이제
인증 시점에 검사한다 — IMAP은 `imap`, 제출은 `smtp`.

- **빈 목록 = 전체 허용.** 스코프가 생기기 전에 발급된 비밀번호를 업그레이드
  하자마자 전부 잠가버리면, 그 좁은 권한으로 얻는 것보다 잃는 게 크다.
- 비밀번호는 맞는데 스코프가 없으면 `ErrScopeDenied` — **브루트포스 카운터를
  올리지 않는다.** 올리면 정직한 클라이언트가 읽기 전용 비밀번호로 발송을
  시도했다는 이유로 15분 잠긴다.

### 일일 한도

검사와 증가를 한 문장(UPSERT … RETURNING)으로 한다. 읽고 나서 쓰면 "99/100"을
동시에 읽은 두 발송이 둘 다 통과한다. 거부는 4xx(451/429)다 — 한도는 내일
풀리므로 영구 실패로 돌려보내면 안 된다. 트랜잭션이 뒤에서 실패하면 센 만큼
돌려준다(`ReleaseSendCounter`).
